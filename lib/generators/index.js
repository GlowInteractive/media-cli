...
async function generateCampaign(unitToGenerate) {
  ...
  return new Promise(async (res, rej) => {
    try {
      ...
      const hasIsi = files.directoryExists(templateAbsPath + "/assets/isi.ejs");

      if (unitToGenerate.references && unitToGenerate.lmr && hasIsi) {
        const isi = await unit.renderHTML({
          templateData: { ...unitToGenerate },
          templateFilePath: templateAbsPath + "/assets/isi.ejs"
        });

        await unit.copyAndWriteUnit(templateAbsPath, unitName, h);
